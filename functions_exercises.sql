...
SELECT 
DATEDIFF(CURDATE(),hire_date) AS days_worked, CONCAT(first_name, " " , last_name) 
AS full_name FROM employees WHERE hire_date BETWEEN '1990-01-01' AND '1999-12-31' 
AND(birth_date LIKE '%-12-25');
    
-- Find the smallest and largest current salary from the salaries table.

SELECT MIN(salary) 
AS minimum_salary,MAX(salary) 
AS max_salary 
FROM salaries;
